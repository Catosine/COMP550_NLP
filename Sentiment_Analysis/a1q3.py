...
class FeatureExtractor():
    ...
    def __extract_glove_vectors__(self, data):
        if not self.glove_dict:
            logging.info('-'*20)
            logging.info('Prepare GloVe data')
            with open(self.glove_dir, 'r') as f:
                for line in tqdm(f):
                    splitLine = line.split()
                    word = splitLine[0]
                    embedding = np.array([float(val) for val in splitLine[1:]])
                    self.glove_dict[word] = embedding

        size = len(data[0])
        logging.info('-'*20)
        logging.info('Extract GloVe data')
        for idx in tqdm(range(size)):
            feature = np.zeros(self.glove_size)
            for word in data[0][idx]:
                try:
                    feature += self.glove_dict[word]
                except:
                    feature += np.zeros(self.glove_size)
            data[0][idx] = feature
    ...
    def __extract_tfidf_feature__(self, data, dictionary):
        size = len(data[0])
        if self.existance_dict is None:
            self.existance_dict = self.__count_existance__(data, dictionary.keys())
        total_corpus = len(self.existance_dict)
        logging.info('-'*20)
        logging.info('Prepare tfidf data')
        for i in tqdm(range(size)):
            tfidf = list()
            for word, frequency in self.existance_dict.items():
                sentence = data[0][i]
    ...
    def __count_existance__(self, data, diction):
        logging.info('-'*20)
        logging.info('Prepare existance dictionary')
        existance_dict = dict()
        for sentence in tqdm(data[0]):
            for word in diction:
                if word in sentence:
                    try:
                        existance_dict[word] += 1
                    except:
                        existance_dict[word] = 1

        return existance_dict
    ...
    def extract_feature(self, data):
        ...
        elif 'tfidf' in self.list_of_process:
            data = self.__extract_tfidf_feature__(data, self.dictionary)
        else:
...
def main():
    ...
    logging.info('Trainset Accuracy = %f', train_acc)
    logging.info('Testset Accuracy = %f', result['micro_avg_recall'])
    logging.info('-' * 20)
